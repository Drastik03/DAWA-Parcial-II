...
class MedicalHistoryComponent:
    ...
    def insertMedicalHistory(data):
        try:
            ...
            if result_db.get("result") and result_db.get("data"):
                new_hist_id = result_db["data"][0]["hist_id"]
                return internal_response(True, "Historial médico insertado exitosamente", {'hist_id': new_hist_id})
            else:
                return internal_response(False, result_db.get("message", "Error al insertar historial médico"),
                                         None)
    ...
    @staticmethod
    def getMedicalHistoriesByPatientId(hist_patient_id: int):
        """
        Obtiene todos los historiales médicos asociados a un ID de paciente específico.
        """
        try:
            HandleLogs.write_log(
                f"Obteniendo historiales médicos para paciente ID: {hist_patient_id} (MedicalHistoryComponent).")
            ...
            if result_db.get('result'):
                ...
                if data_from_db:
                    ...
                    return internal_response(False,
                                             f"No se encontraron historiales médicos para el paciente ID: {hist_patient_id}",
                                             None)
            else:
                # Si la operación a la base de datos falló
                message = result_db.get('message',
                                        f"Error en la base de datos al obtener historiales para paciente ID: {hist_patient_id}.")
                HandleLogs.write_error(f"Error en DBHandle.getRecords para patient_id {hist_patient_id}: {message}")
                return internal_response(False, message, None)
