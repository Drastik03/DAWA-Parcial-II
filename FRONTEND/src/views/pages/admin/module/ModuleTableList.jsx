...
import { updateModule, deleteModule } from "../../../../services/admin/moduleService"; // Asegúrate de tener estas funciones

function Row({ row, onDelete, onEdit }) {
    return (
        <TableRow hover>
            <TableCell>
                <Typography variant="h6" fontWeight="600">{row.mod_name}</Typography>
            </TableCell>
            <TableCell>
...
const ModuleTableList = () => {
    ...
    const handleConfirmDelete = async () => {
        try {
            ...
            if (res.result) {
                setAlert({ open: true, message: "Módulo eliminado", severity: "success" });
                refetch();
            } else {
                throw new Error();
            }
        } catch {
            setAlert({ open: true, message: "Error al eliminar módulo", severity: "error" });
        } finally {
    ...
    const handleEditSave = async () => {
        try {
            ...
            if (res.result) {
                setAlert({ open: true, message: "Módulo actualizado", severity: "success" });
                refetch();
            ...
            setAlert({ open: true, message: "Error al actualizar módulo", severity: "error" });
        }
    };
    ...
    return (
        <PageContainer title="Módulos" description="Listado de módulos">
            ...
            {alert.open && (
                <Box mb={2}>
                    <Alert severity={alert.severity} onClose={() => setAlert({ ...alert, open: false })} variant="filled">
                        {alert.message}
                    </Alert>
                </Box>
            )}
                        ...
                        <Table aria-label="module table">
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell><Typography variant="h6">Orden</Typography></TableCell>
                                    <TableCell><Typography variant="h6">Icono</Typography></TableCell>
                                    <TableCell><Typography variant="h6">Texto</Typography></TableCell>
                                    <TableCell align="center"><Typography variant="h6">Acciones</Typography></TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {rows.map((row) => (
                                    <Row key={row.mod_id} row={row} onDelete={handleDeleteClick} onEdit={handleEditClick} />
                                ))}
                            </TableBody>
                        </Table>
                    </TableContainer>
            ...
            <Dialog open={openDeleteModal} onClose={handleCloseDeleteModal}>
                ...
                <DialogContent>
                    <DialogContentText>
                        ¿Está seguro que desea eliminar el módulo <strong>{selectedRow?.mod_name}</strong>?
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
